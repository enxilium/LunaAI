...
def configure_uvicorn_logging():
    """Configure uvicorn to send INFO logs to stdout instead of stderr"""
    ...
    # Create stdout handler for INFO and above
...
# Configure uvicorn logging immediately when module is imported
...
class StreamingServer:
    ...
    async def initialize_pre_warmed_components(self):
        ...
        if pre_warmed_runner is None:
            log_info("[STREAMING] Pre-warming Runner for faster startup...")
            pre_warmed_runner = Runner(
                app_name=APP_NAME,
                agent=root_agent,
                session_service=session_service,
            )
            log_info("[STREAMING] Runner pre-warming complete")
    ...
    def setup_routes(self):
        ...
        async def websocket_endpoint(websocket: WebSocket, client_id: str):
            ...
            log_info(f"[STREAMING] Client {client_id} connected for multimodal streaming")
            ...
            try:
                ...
                log_info(f"[STREAMING] Session initialized in {session_init_time:.3f}s for client {client_id}")
                ...
                log_info(f"[STREAMING] Total startup time: {total_startup_time:.3f}s for client {client_id}")
    ...
    async def start_agent_session(self, user_id: str):
        ...
        if pre_warmed_runner is None:
            ...
            log_info("[STREAMING] Using pre-warmed Runner")
    ...
    async def agent_to_client_messaging(self, websocket: WebSocket, live_events):
        ...
        try:
            ...
            log_error(f"[STREAMING] Error in agent_to_client_messaging: {e}")
    ...
    def start_server(self, host: str = "localhost", port: int = PORT):
        ...
        log_info(f"[STREAMING] Starting Luna AI Streaming Server on {host}:{port}")
        
        # Pre-warm components before starting server
        import asyncio
        asyncio.run(self.initialize_pre_warmed_components())
        
        # Create custom logging config for uvicorn to use stdout
        log_config = {
            "version": 1,
